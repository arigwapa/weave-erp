...
export default function BudgetPlannerPage() {
  ...
  const loadBudgetRows = async () => {
    ...
    try {
      ...
      const pickerCandidates = mapped.filter(
        (item) => item.plannerStatus.trim().toLowerCase() === "planning budget",
      );
                ...
                >
                  ...
                  <TableCell className="text-right">
                    <div className="flex justify-end gap-2">
                      ...
                      >
                        {row.isAdminApproved ? "View Budget Plan" : "Add Budget Plan"}
                      </SecondaryButton>
                    </div>
                  </TableCell>
                </TableRow>
      ...
      <DetailsModal
        ...
        footerActions={
          <PrimaryButton
            ...
            isLoading={isSaving}
            disabled={!selectedCollection || !!selectedCollection?.isAdminApproved}
          >
            {selectedCollection?.isAdminApproved ? "Budget Locked (Admin Approved)" : "Save Budget Plan"}
          </PrimaryButton>
        }
      >
        <div className="grid gap-3 sm:grid-cols-2">
          ...
          {selectedCollection?.isAdminApproved ? (
            <p className="sm:col-span-2 text-xs text-slate-500">
              This collection is admin approved. Values are view-only.
            </p>
          ) : null}
        </div>
      </DetailsModal>
