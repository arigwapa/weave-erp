...
export default function ProfilePage() {
  ...
  const [username] = useState(user?.username ?? "superadmin");
  ...
  const avatarInitials = (user?.fullname || "Super Admin")
    .split(" ")
    .filter(Boolean)
    .slice(0, 2)
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col items-center text-center py-2">
        <div className="h-28 w-28 rounded-full bg-indigo-100 border-4 border-white shadow-md flex items-center justify-center text-3xl font-bold text-indigo-700">
          {avatarInitials}
        </div>
        <p className="mt-4 text-lg font-semibold text-slate-900">{user?.fullname || "Super Admin"}</p>
        <p className="text-sm text-slate-500">{user?.roleName || "SuperAdmin"}</p>
      </div>
        ...
        <div className="grid gap-4 xl:grid-cols-3">
          <Card className="rounded-2xl xl:col-span-2">
            ...
            <CardContent className="grid gap-4 sm:grid-cols-2">
              ...
              <div className="space-y-2">
                <Label>Role</Label>
                <Input className="rounded-2xl" value={user?.roleName ?? "SuperAdmin"} disabled />
              </div>
              <div className="space-y-2">
          ...
          <Card className="rounded-2xl">
            ...
            <CardContent className="space-y-3">
              <div className="rounded-xl border border-slate-200 p-3">
                ...
                <p className="mt-1 text-sm font-semibold text-slate-800">{user?.username ?? "superadmin"}</p>
              </div>
              <div className="rounded-xl border border-slate-200 p-3">
      ...
      {activeTab === "preferences" && (
        <Card className="rounded-2xl">
          ...
          <CardContent className="space-y-3">
            <div className="rounded-xl border border-slate-200 p-3 flex items-center justify-between">
              <div>
                <p className="text-sm font-medium text-slate-800">Security Digest</p>
                <p className="text-xs text-slate-500">Receive weekly summary of access and policy updates.</p>
              </div>
              <ToggleSwitch
                active={weeklyDigest}
