...
namespace weave_erp_backend_api.Controllers
{
    ...
    public class ProductionBatchBoardController : ControllerBase
    {
        ...
        {
            var runCandidateSchedules = await _context.RunSchedules
                .AsNoTracking()
                .Where(x =>
                    x.Status == "Run Candidate" ||
                    x.Status == "Schedule Running")
                .OrderByDescending(x => x.UpdatedAt ?? x.CreatedAt)
                .ToListAsync();

            if (runCandidateSchedules.Count == 0)
            {
            ...
            var candidates = runCandidateSchedules
                .Select(schedule =>
                {
        ...
        public async Task<ActionResult<IEnumerable<ProductionBatchBoardItemDto>>> Save([FromBody] SaveProductionBatchBoardRequest request)
        {
            ...
            foreach (var dto in items)
            {
                ...
                if (!IsEligibleBatchSourceStatus(linkedSchedule.Status))
                {
                    invalidCreateReasons.Add(
                        $"Batch '{normalizedCode}' cannot be created because run '{scheduleKey}' is '{NormalizeSourceStatus(linkedSchedule.Status)}' (must be 'Run Candidate' or 'Unpacked')."
                    );
                }
            }
            ...
            if (invalidCreateReasons.Count > 0)
            {
                ...
                {
                    message = "Batch creation requires linked run status 'Run Candidate' or 'Unpacked'.",
            ...
            foreach (var dto in items)
            {
                ...
                if (string.Equals(entity.Status, "For Inspection", StringComparison.OrdinalIgnoreCase))
                {
                    entity.QAStatus = "For Inspection";
                    entity.QAStartedAt = null;
                    entity.QAStartedByUserID = null;
                }
        ...
        private static bool IsEligibleBatchSourceStatus(string? status)
        {
            var value = (status ?? string.Empty).Trim();
            return string.Equals(value, "Run Candidate", StringComparison.OrdinalIgnoreCase)
                || string.Equals(value, "Unpacked", StringComparison.OrdinalIgnoreCase)
                || string.Equals(value, "Batch Ready", StringComparison.OrdinalIgnoreCase)
                || string.Equals(value, "Schedule Running", StringComparison.OrdinalIgnoreCase);
        }
