...
namespace weave_erp_backend_api.Controllers
{
    ...
    public class BudgetsController : GenericCrudController<Budget>
    {
        ...
        [HttpPost("{budgetId:int}/reserve-for-version/{versionId:int}")]
        public async Task<ActionResult<BudgetReservation>> ReserveForVersion(int budgetId, int versionId)
        {
            ...
            var versionExists = await _context.ProductionVersions.AnyAsync(x => x.VersionID == versionId);
            if (!versionExists)
            {
                return NotFound("Product version not found.");
            }
