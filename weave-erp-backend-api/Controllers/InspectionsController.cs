...
namespace weave_erp_backend_api.Controllers
{
    ...
    public class InspectionsController : GenericCrudController<Inspection>
    {
        ...
        public async Task<ActionResult<InspectionDetailDto>> GetDetails(int id)
        {
            ...
            var attachments = await _context.InspectionAttachments
                .AsNoTracking()
                .Where(x => x.InspectionID == inspection.InspectionID)
                .OrderByDescending(x => x.UploadedAt)
                .Select(x => new InspectionAttachmentDetailDto
                {
                    InspectionAttachmentID = x.InspectionAttachmentID,
                    FileName = x.FileName,
                    FileUrl = x.FileUrl,
                    FileType = x.FileType,
                    Remarks = x.Remarks
        ...
        private async Task<int> CreateLegacyProductionBatchRowAsync(ProductionBatchBoardItem batch, int userId)
        {
            var resolvedOrderId = await ResolveLegacyOrderIdAsync(batch.OrderID);
            if (resolvedOrderId <= 0)
            {
                throw new InvalidOperationException("Unable to resolve a valid ProductionOrder for legacy ProductionBatch insert.");
            }

            var baseBatchNumber = string.IsNullOrWhiteSpace(batch.Code)
                ? $"BATCH-{batch.BatchBoardID}"
                : batch.Code.Trim();
            if (baseBatchNumber.Length > 80)
            {
                baseBatchNumber = baseBatchNumber[..80];
            }

            string selectedBatchNumber = baseBatchNumber;
            for (var i = 0; i < 20; i++)
            {
                selectedBatchNumber = i == 0 ? baseBatchNumber : BuildLegacyBatchNumber(baseBatchNumber, i);
                var exists = await _context.ProductionBatches
                    .AsNoTracking()
                    .AnyAsync(x => x.BatchNumber == selectedBatchNumber);
                if (!exists)
                {
                    break;
                }
            ...
            try
            {
                ...
                cmd.CommandText = @"
INSERT INTO [ProductionBatch] ([OrderID],[BatchNumber],[BatchQty],[ProducedDate],[Status],[CreatedByUserID],[CreatedAt],[UpdatedByUserID],[UpdatedAt])
VALUES (@orderId,@batchNumber,@batchQty,@producedDate,@status,@createdBy,@createdAt,@updatedBy,@updatedAt);
SELECT CAST(SCOPE_IDENTITY() AS int);";

                AddSqlParam(cmd, "@orderId", resolvedOrderId);
                AddSqlParam(cmd, "@batchNumber", selectedBatchNumber);
                AddSqlParam(cmd, "@batchQty", Math.Max(0, batch.Qty));
                AddSqlParam(cmd, "@producedDate", DateTime.UtcNow.Date);
                AddSqlParam(cmd, "@status", "In Progress");
                AddSqlParam(cmd, "@createdBy", userId);
                AddSqlParam(cmd, "@createdAt", DateTime.UtcNow);
                AddSqlParam(cmd, "@updatedBy", userId);
                AddSqlParam(cmd, "@updatedAt", DateTime.UtcNow);
        ...
        private async Task<int> ResolveLegacyOrderIdAsync(int preferredOrderId)
        {
            var exists = await _context.ProductionOrders.AsNoTracking().AnyAsync(x => x.OrderID == preferredOrderId);
            if (exists)
            {
        ...
        private static string BuildLegacyBatchNumber(string baseBatchNumber, int suffix)
        {
            var suffixText = $"-{suffix}";
            var maxBaseLen = Math.Max(1, 80 - suffixText.Length);
            var trimmed = baseBatchNumber.Length > maxBaseLen ? baseBatchNumber[..maxBaseLen] : baseBatchNumber;
            return $"{trimmed}{suffixText}";
        }

        private static void AddSqlParam(System.Data.Common.DbCommand command, string name, object? value)
        {
