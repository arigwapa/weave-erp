...
export default function BranchProfilePage() {
  ...
  const [fullName, setFullName] = useState(user?.fullname ?? "Branch Manager");
  const [username] = useState(user?.username ?? "branchmanager");
  const [mobile, setMobile] = useState("+63 9xx xxx xxxx");
  const [jobTitle, setJobTitle] = useState("Branch Operations Manager");
  ...
  const avatarInitials = (user?.fullname || "Branch Manager")
    .split(" ")
    .filter(Boolean)
    .slice(0, 2)
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col items-center py-2 text-center">
        ...
        <p className="mt-4 text-lg font-semibold text-slate-900">{user?.fullname || "Branch Manager"}</p>
        <p className="text-sm text-slate-500">{user?.roleName || "BranchManager"}</p>
      </div>
        ...
        <div className="grid gap-4 xl:grid-cols-3">
          <Card className="rounded-2xl xl:col-span-2">
            ...
            <CardContent className="grid gap-4 sm:grid-cols-2">
              ...
              <div className="space-y-2">
                <Label>Role</Label>
                <Input className="rounded-2xl" value={user?.roleName ?? "BranchManager"} disabled />
              </div>
              <div className="space-y-2">
          ...
          <Card className="rounded-2xl">
            <CardHeader>
              ...
              <CardDescription>Current profile and branch workspace status.</CardDescription>
            </CardHeader>
            <CardContent className="space-y-3">
              <div className="rounded-xl border border-slate-200 p-3">
                <p className="text-xs text-slate-500">Username</p>
                <p className="mt-1 text-sm font-semibold text-slate-800">{user?.username ?? "branchmanager"}</p>
              </div>
              <div className="rounded-xl border border-slate-200 p-3">
                <p className="text-xs text-slate-500">Branch Scope</p>
                <p className="mt-1 text-sm font-semibold text-slate-800">{user?.branchName || "Branch Portal"}</p>
              </div>
              <div className="rounded-xl border border-slate-200 p-3">
          ...
          <CardContent className="space-y-3">
            <div className="flex items-center justify-between rounded-xl border border-slate-200 p-3">
              <div>
                ...
                <p className="text-xs text-slate-500">Require MFA for all branch sign-ins.</p>
              </div>
            ...
            <div className="flex items-center justify-between rounded-xl border border-slate-200 p-3">
              <div>
                ...
                <p className="text-xs text-slate-500">Send alert when account signs in from a new device.</p>
              </div>
              <ToggleSwitch
                active={loginAlerts}
      ...
      {activeTab === "preferences" && (
        <Card className="rounded-2xl">
          <CardHeader>
            ...
            <CardDescription>Choose how and when branch updates are delivered.</CardDescription>
          </CardHeader>
          <CardContent className="space-y-3">
            <div className="flex items-center justify-between rounded-xl border border-slate-200 p-3">
              <div>
                <p className="text-sm font-medium text-slate-800">Operations Digest</p>
                <p className="text-xs text-slate-500">Receive weekly summary of branch request and receiving activity.</p>
              </div>
              <ToggleSwitch
                active={weeklyDigest}
                onToggle={() => setWeeklyDigest((v) => !v)}
                label="Toggle operations digest"
              />
            </div>
            <div className="rounded-xl border border-slate-200 p-3">
