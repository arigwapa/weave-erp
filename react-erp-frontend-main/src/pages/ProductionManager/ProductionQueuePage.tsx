...
type QueueLifecycleStatus = QueueStatus | "For Scheduling" | "Schedule Ready" | "Schedule Running" | "Finished Run";
...
const resolveProductStatus = (
  ...
  if (schedule) {
    ...
    if (schedule.status === "Schedule Running") return { displayStatus: "Schedule Running", tabStatus: "On Going" };
...
export default function ProductionQueuePage() {
    ...
    () => [
      {
        ...
        label: "Pending",
        icon: Clock3,
        count: queueRows.filter((r) => r.tabStatus === "Pending").length,
      },
      {
        id: "On Going",
        label: "On Going",
  ...
  const filteredRows = useMemo(
    () =>
      queueRows.filter((row) => {
        const inTab = row.tabStatus === activeTab;
  ...
  const addSchedule = (row: ProductQueueRow) => {
    ...
    const nextItem: RunScheduleRecord = {
      key: buildScheduleKey(product.CollectionID, product.ProductID),
    ...
    const byKey = new Map<string, RunScheduleRecord>(schedules.map((item) => [item.key, item]));
          ...
          <Table>
            ...
            <TableBody>
              {filteredRows.map((row) => (
                <TableRow key={`${row.item.CollectionID}-${row.item.ProductID}`}>
