...
export default function ProductionProfilePage() {
  ...
  const [fullName, setFullName] = useState(user?.fullname ?? "Production Manager");
  const [username] = useState(user?.username ?? "productionmanager");
  const [mobile, setMobile] = useState("+63 9xx xxx xxxx");
  const [jobTitle, setJobTitle] = useState("Production Operations Lead");
  ...
  const avatarInitials = (user?.fullname || "Production Manager")
    .split(" ")
    .filter(Boolean)
    .slice(0, 2)
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col items-center py-2 text-center">
        ...
        <p className="mt-4 text-lg font-semibold text-slate-900">{user?.fullname || "Production Manager"}</p>
        <p className="text-sm text-slate-500">{user?.roleName || "ProductionManager"}</p>
      </div>
        ...
        <div className="grid gap-4 xl:grid-cols-3">
          <Card className="rounded-2xl xl:col-span-2">
            ...
            <CardContent className="grid gap-4 sm:grid-cols-2">
              ...
              <div className="space-y-2">
                <Label>Role</Label>
                <Input className="rounded-2xl" value={user?.roleName ?? "ProductionManager"} disabled />
              </div>
              <div className="space-y-2">
          ...
          <Card className="rounded-2xl">
            ...
            <CardContent className="space-y-3">
              <div className="rounded-xl border border-slate-200 p-3">
                ...
                <p className="mt-1 text-sm font-semibold text-slate-800">{user?.username ?? "productionmanager"}</p>
              </div>
              <div className="rounded-xl border border-slate-200 p-3">
                <p className="text-xs text-slate-500">Branch Scope</p>
                <p className="mt-1 text-sm font-semibold text-slate-800">{user?.branchName || "Global"}</p>
              </div>
              <div className="rounded-xl border border-slate-200 p-3">
          ...
          <CardContent className="space-y-3">
            <div className="flex items-center justify-between rounded-xl border border-slate-200 p-3">
              <div>
                ...
                <p className="text-xs text-slate-500">Require MFA for all production sessions.</p>
              </div>
          ...
          <CardContent className="space-y-3">
            <div className="flex items-center justify-between rounded-xl border border-slate-200 p-3">
              <div>
                ...
                <p className="text-xs text-slate-500">Receive weekly summary of production and QA handoff updates.</p>
              </div>
              <ToggleSwitch
                active={weeklyDigest}
