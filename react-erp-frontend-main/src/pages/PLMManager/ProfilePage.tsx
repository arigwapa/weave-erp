...
export default function PLMProfilePage() {
  const { user } = useAuth();
  const [activeTab, setActiveTab] = useState<ProfileTab>("account");
  const [fullName, setFullName] = useState(user?.fullname ?? "PLM Manager");
  const [username] = useState(user?.username ?? "plmmanager");
  const [mobile, setMobile] = useState("+63 9xx xxx xxxx");
  const [jobTitle, setJobTitle] = useState("Product Lifecycle Manager");
  ...
  const avatarInitials = (user?.fullname || "PLM Manager")
    .split(" ")
    .filter(Boolean)
    .slice(0, 2)
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col items-center py-2 text-center">
        ...
        <p className="mt-4 text-lg font-semibold text-slate-900">{user?.fullname || "PLM Manager"}</p>
        <p className="text-sm text-slate-500">{user?.roleName || "PLMManager"}</p>
      </div>
        ...
        <div className="grid gap-4 xl:grid-cols-3">
          <Card className="rounded-2xl xl:col-span-2">
            ...
            <CardContent className="grid gap-4 sm:grid-cols-2">
              ...
              <div className="space-y-2">
                <Label>Role</Label>
                <Input className="rounded-2xl" value={user?.roleName ?? "PLMManager"} disabled />
              </div>
              <div className="space-y-2">
          ...
          <Card className="rounded-2xl">
            ...
            <CardContent className="space-y-3">
              <div className="rounded-xl border border-slate-200 p-3">
                ...
                <p className="mt-1 text-sm font-semibold text-slate-800">{user?.username ?? "plmmanager"}</p>
              </div>
              <div className="rounded-xl border border-slate-200 p-3">
          ...
          <CardContent className="space-y-3">
            <div className="flex items-center justify-between rounded-xl border border-slate-200 p-3">
              <div>
                ...
                <p className="text-xs text-slate-500">Require MFA for all PLM sessions.</p>
              </div>
      ...
      {activeTab === "preferences" && (
        <Card className="rounded-2xl">
          ...
          <CardContent className="space-y-3">
            <div className="flex items-center justify-between rounded-xl border border-slate-200 p-3">
              <div>
                <p className="text-sm font-medium text-slate-800">Workflow Digest</p>
                <p className="text-xs text-slate-500">Receive weekly summary of timeline and submission updates.</p>
              </div>
              <ToggleSwitch
                active={weeklyDigest}
                onToggle={() => setWeeklyDigest((v) => !v)}
                label="Toggle workflow digest"
              />
            </div>
            <div className="rounded-xl border border-slate-200 p-3">
