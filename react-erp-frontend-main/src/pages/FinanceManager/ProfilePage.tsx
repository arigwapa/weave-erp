...
export default function FinanceProfilePage() {
  ...
  const [fullName, setFullName] = useState(user?.fullname ?? "Finance Manager");
  const [username] = useState(user?.username ?? "financemanager");
  const [mobile, setMobile] = useState("+63 9xx xxx xxxx");
  const [jobTitle, setJobTitle] = useState("Budget and Cost Controller");
  ...
  const avatarInitials = (user?.fullname || "Finance Manager")
    .split(" ")
    .filter(Boolean)
    .slice(0, 2)
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col items-center py-2 text-center">
        ...
        <p className="mt-4 text-lg font-semibold text-slate-900">{user?.fullname || "Finance Manager"}</p>
        <p className="text-sm text-slate-500">{user?.roleName || "FinanceManager"}</p>
      </div>
        ...
        <div className="grid gap-4 xl:grid-cols-3">
          <Card className="rounded-2xl xl:col-span-2">
            ...
            <CardContent className="grid gap-4 sm:grid-cols-2">
              ...
              <div className="space-y-2">
                <Label>Role</Label>
                <Input className="rounded-2xl" value={user?.roleName ?? "FinanceManager"} disabled />
              </div>
              <div className="space-y-2">
          ...
          <Card className="rounded-2xl">
            ...
            <CardContent className="space-y-3">
              <div className="rounded-xl border border-slate-200 p-3">
                ...
                <p className="mt-1 text-sm font-semibold text-slate-800">{user?.username ?? "financemanager"}</p>
              </div>
              <div className="rounded-xl border border-slate-200 p-3">
          ...
          <CardContent className="space-y-3">
            <div className="flex items-center justify-between rounded-xl border border-slate-200 p-3">
              <div>
                ...
                <p className="text-xs text-slate-500">Require MFA for all finance sessions.</p>
              </div>
      ...
      {activeTab === "preferences" && (
        <Card className="rounded-2xl">
          ...
          <CardContent className="space-y-3">
            <div className="flex items-center justify-between rounded-xl border border-slate-200 p-3">
              <div>
                <p className="text-sm font-medium text-slate-800">Finance Digest</p>
                <p className="text-xs text-slate-500">Receive weekly summary of budget and variance updates.</p>
              </div>
              <ToggleSwitch
                active={weeklyDigest}
                onToggle={() => setWeeklyDigest((v) => !v)}
                label="Toggle finance digest"
              />
            </div>
            <div className="rounded-xl border border-slate-200 p-3">
